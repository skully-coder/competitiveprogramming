...
export default function Category(props) {
	...
	useEffect(() => {
		axios
			.get(category.url + "?recursive=1")
			.then(response => {
				const problems = response.data.tree.reduce((accumulator, object) => {
					if (object.type === "blob") {
						const parts = object.path.split("/")
						const name =
							parts[1].toUpperCase() === "README.MD" ? "readme" : parts[1].split(".")[1].toLowerCase()
	...
	return (
		<div className="col-12 col-md-10">
			<div className="container text-center">
				...
				<div
					className="table-responsive overflow-auto shadow-lg mx-auto"
					style={{
						maxHeight: "65vh",
						width: "90%",
					}}>
					<table className="table table-bordered table-hover">
						...
						<tbody>
							{Object.entries(problems).map(([title, data], index) => (
								<Problem
									key={index}
									title={title}
									data={data}
			...
			<tr>
				...
				<td>
					<button
						...
						onClick={() => props.showCode(props.data.readme, false)}
						data-toggle="modal"
						data-target="readme">
				...
				<td>
					<a
						...
						href={
							"https://github.com/skully-coder/competitiveprogramming/tree/main/" +
							props.category +
							"/" +
							props.title
						}>
