...
function Problem(props) {
	const icons = (language) => {
		switch(language) {
			...
			case "java":
				return <FontAwesomeIcon className="font-size-22" icon={faJava} />
			case "py":
				return <FontAwesomeIcon className="font-size-18" icon={faPython} />
			case "js":
				return <FontAwesomeIcon className="font-size-18" icon={faJsSquare} />
			default:
		...
		<tr>
			...
			<td>
				{Object.keys(props.data).map((value, index) => value === "readme" ? null :
					<button
						key={index} data-toggle="modal" data-target="source-code"
						className="btn btn-success font-weight-semi-bold mr-5 px-10"
						onClick={() => props.showCode(props.data[value], true)}
					>
						{icons(value)}
					</button>
				)}
			...
			<td>
				<button className="btn btn-primary" onClick={() => props.showCode(props.data.readme, false)} data-toggle="modal" data-target="readme">
					<FontAwesomeIcon icon={faFileAlt} />
				</button>
			</td>
			<td>
				<a target="_blank" rel="noreferrer" className="btn btn-secondary" href={"https://github.com/skully-coder/hacktoberfest-competitiveprogramming/tree/main/" + props.category + "/" + props.title}>
